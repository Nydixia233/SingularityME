...
`NetworkUiKit.colorSwatchRow`（`src/main/java/com/github/singularityme/client/ui/NetworkUiKit.java:1041`）旧实现把可见色块背景画在 `ButtonWidget` 的内部 `Flow` 上，而 `ButtonWidget` 本体背景是 `IDrawable.NONE`。
...
- 在 `NetworkUiKitTest.rendersColorSwatchesOnClickableButtonItself`（`src/test/java/com/github/singularityme/client/ui/NetworkUiKitTest.java:420`）中断言：色块按钮本体背景和 overlay 可见，且不再包含会抢占命中的子节点。
