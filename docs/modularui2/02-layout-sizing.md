...
`IPositioned<W>` 的源码签名返回泛型 `W`，但在具体 Widget、raw type 或链式推断场景下，Java 静态类型可能退化为 `IPositioned`，导致 `.width()` 之后不能继续调用 Widget 专属方法。不要把这个现象写成“API 直接返回 `IPositioned`”：2.3.63 的 `javap` 擦除签名会显示 `IPositioned`，源码泛型签名仍是 `W`。旧 guide 中已验证规避方式：
